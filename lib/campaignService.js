...
class CampaignService {
  ...
  static async getCampaigns(merchantId, filters = {}) {
    // Keep stored status in sync with current dates. The two conditions are
    // mutually exclusive, so they can run concurrently instead of serially.
    const now = new Date();
    await Promise.all([
      Campaign.updateMany(
        { merchantId, status: { $ne: 'ended' }, endDate: { $lt: now } },
        { status: 'ended' }
      ),
      Campaign.updateMany(
        { merchantId, status: 'ended', endDate: { $gt: now } },
        { status: 'active' }
      ),
    ]);
    ...
    // .lean() avoids hydrating full Mongoose documents for what's a read-only
    // list — the rare stale-store cleanup below writes via targeted
    // updateOne calls instead of needing live documents to .save().
    ...
    // Independent lookups — run concurrently rather than one after another.
    const [existingStores, ranges] = await Promise.all([
      allStoreIds.size > 0
        ? Store.find({ _id: { $in: Array.from(allStoreIds) } }).select('_id').lean()
        : Promise.resolve([]),
      Range.find({ campaignId: { $in: campaignIds } }).lean(),
    ]);

    const validStoreIds = new Set(existingStores.map(s => s._id.toString()));
    ...
    // Persist stale-reference cleanup for just the affected campaigns (rare).
    if (campaignsNeedingUpdate.length > 0) {
      await Promise.all(
        campaignsNeedingUpdate.map(c =>
          Campaign.updateOne({ _id: c._id }, { assignedStores: c.assignedStores })
        )
      );
    }
  ...
  static async deleteCampaign(campaignId) {
    ...
    // Guard: refuse to delete a campaign with real customer activity —
    // deleting it would orphan scratch card records, participations,
    // scans, and allocation requests that reference this campaign with
    // no way to recover the context.
    const [scratchRecords, participations, scans, allocationRequests] = await Promise.all([
      ScratchCardRecord.countDocuments({ campaign_id: campaignId }),
      CustomerParticipation.countDocuments({ campaign_id: campaignId }),
      Scan.countDocuments({ campaignId }),
      ScratchAllocationRequest.countDocuments({ campaignId }),
    ]);
    if (scratchRecords + participations + scans + allocationRequests > 0) {
      throw new ValidationError(
        'Cannot delete a campaign that already has customer activity (scratches, scans, or participations).'
      );
    }
    ...
    // Safe to hard-delete ranges tied to this campaign — nothing else can
    // reference them since we already confirmed there's no customer
    // activity on this campaign.
    const rangeCleanup = await Range.deleteMany({ campaignId });
    let couponRangesRemoved = 0;
    if (CouponRange) {
      const couponRangeCleanup = await CouponRange.deleteMany({ campaignId });
      couponRangesRemoved = couponRangeCleanup.deletedCount || 0;
    }
    ...
    return {
      ...
      cascadeCleanupStats: {
        ...
        rangesRemoved: rangeCleanup.deletedCount || 0,
        couponRangesRemoved,
