...
try {
  redis = new Redis({
    ...
    retryStrategy: (times) => {
      if (times === 1) {
        console.warn('Redis unavailable — OTP rate limiting disabled.');
      }
...
class OTPService {
  ...
  async generateOTP(phone, purpose, ipAddress = null, userAgent = null) {
    ...
    // Rate limiting - Redis-based (skip if Redis unavailable)
    if (redis) {
      try {
        ...
        console.warn('Rate limit check failed (Redis unavailable):', err.message);
    ...
    // Set rate limit (1 minute) — if Redis fails, continue anyway
    if (redis) {
      try {
        await redis.setex(rateLimitKey, 60, Date.now().toString());
      } catch (err) {
        console.warn('Failed to set rate limit in Redis:', err.message);
        // Don't fail the OTP send due to Redis issues
      }
    }
