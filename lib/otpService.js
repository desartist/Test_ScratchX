...
class OTPService {
  ...
  async generateOTP(phone, purpose, ipAddress = null, userAgent = null) {
    ...
    if (redis) {
      try {
        lastSentTime = await redis.get(rateLimitKey);
      } catch {
        // Redis unreachable — degrade gracefully, same as a missing client.
      }
    }
    ...
    if (redis) {
      try {
        ...
        // Redis unreachable — rate limit just won't be recorded this time.
      }
    }
