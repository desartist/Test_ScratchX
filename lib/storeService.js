...
class StoreService {
  ...
  static async getStoresByMerchant(merchantId, options = {}) {
    const { page = 1, limit = 10, status = null, searchTerm = "", includeDeleted = false } = options;
    const skip = (page - 1) * limit;

    // Build query
    const query = { merchant_id: merchantId };
    if (!includeDeleted) {
      // Belt-and-suspenders: also exclude by status, since stores soft-deleted
      // before isDeleted/deletedAt existed on the schema only ever got
      // status: 'deleted' persisted (isDeleted was silently stripped by
      // Mongoose's strict mode). Checking both catches that legacy data too.
  ...
  /**
   * Soft-delete a store by ID, with guards and cascade cleanup.
   *
   * This is the single source of truth for "deleting" a store — both the
   * store detail page and the store list page route through this method,
   * so behavior can't drift between the two entry points.
   *
   * Guards (block deletion, don't leave dangling refs):
   * 1. Refuse to delete a merchant's Main Store.
   * 2. Refuse to delete a store that still has active Store_Manager /
   *    Store_Staff accounts assigned to it — they'd be left with a
   *    storeId pointing at nothing.
   *
   * Cascade cleanup (on success):
   * - Every campaign's assignedStores[] entry for this store is marked
   *   status: "removed" (not deleted) so campaign history/allocation
   *   audit trail is preserved.
   * - The store itself is soft-deleted (isDeleted/deletedAt/status), not
   *   hard-deleted, so historical rows in ScratchCardRecord,
   *   CustomerParticipation, ScratchCardTransaction, etc. that reference
   *   store_id keep resolving to a real (flagged) document instead of a
   *   dangling ObjectId.
   *
   * @param {string} storeId - The store ID
  ...
  static async deleteStore(storeId) {
    ...
    // Guard 1: Main Store cannot be deleted.
    const canDelete = await mainStoreService.canDeleteStore(
      storeId,
      store.merchant_id,
    );
    if (!canDelete.allowed) {
      throw new ValidationError(
        canDelete.message || "This store cannot be deleted.",
      );
    }

    // Guard 2: Refuse to delete a store with staff still assigned to it.
    const assignedStaffCount = await Account.countDocuments({
      storeId,
    ...
    if (assignedStaffCount > 0) {
      throw new ValidationError(
        `Reassign or remove ${assignedStaffCount} staff member${assignedStaffCount > 1 ? "s" : ""} from this store before deleting it.`,
      );
    }

    // Cascade: mark this store's assignment as removed in every campaign
    // that references it (preserves history instead of hard-filtering it
    // out of assignedStores[]).
    ...
    for (const campaign of campaignsWithStore) {
      let changed = false;
      for (const assignment of campaign.assignedStores || []) {
        if (
          ...
          assignment.lastModifiedBy = store.merchant_id;
          changed = true;
        }
      }
