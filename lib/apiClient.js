...
// Routes through the logout endpoint (not straight to /auth/login) so the
// stale authToken/sessionId cookies actually get cleared server-side first.
// Otherwise middleware still sees a present authToken on /auth/login and
// bounces the browser right back to a protected route — an infinite loop.
const AUTH_LOGIN_ROUTE = '/api/auth/logout';
