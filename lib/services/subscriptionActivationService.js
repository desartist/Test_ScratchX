...
import { UNLIMITED_SCRATCHES_DURATION_DAYS } from '@/lib/scratchEntitlementService';
  ...
  async activateSubscription(userId, planId, userType = 'merchant', billingCycle = 'monthly', options = {}) {
    try {
      ...
      if (subscription) {
        ...
        // Activate unlimited scratches for a fixed 365-day grant (independent of billing cycle)
        subscription.unlimitedScratches = {
          isActive: true,
          grantedAt: currentPeriodStart,
          validUntil: new Date(
            currentPeriodStart.getTime() +
              UNLIMITED_SCRATCHES_DURATION_DAYS * 24 * 60 * 60 * 1000
          ),
        };
      } else {
