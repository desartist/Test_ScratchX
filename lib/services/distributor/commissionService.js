...
class CommissionService {
   ...
   * Set by Super Admin (Account.profile.commissionRate), read-only for distributor
   */
  async getCommissionPercentage(distributorId) {
    try {
      const distributor = await Account.findById(distributorId).select(
        'profile.commissionRate'
      );
      if (!distributor) {
        throw new Error('Distributor not found');
      }
      return distributor.profile?.commissionRate || 0;
    } catch (error) {
   ...
   */
  calculateBatchPricing(items, commissionPercentage, includeGst = true) {
    try {
      ...
      const gst = includeGst ? Math.round((subtotal * 0.18) * 100) / 100 : 0; // 18% GST
