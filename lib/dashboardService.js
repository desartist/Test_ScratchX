...
class DashboardService {
  ...
  async getSuperAdminDashboard() {
    try {
      const [
        ...
        Account.countDocuments({ role: "Merchant" }),
        Account.countDocuments({ role: "Manager" }),
        Account.countDocuments({ status: "active" }),
        Account.countDocuments({ status: "pending" }),
        Account.find({ role: "Merchant" })
          .select("name email firstName lastName role status createdAt")
          .sort({ createdAt: -1 })
          .limit(5),
        ...
        // Campaign.{allocated,used}_scratch_cards is the real, live source
        // of platform scratch numbers — Account.scratchCards is a legacy
        // field that stays at 0 for real merchants (confirmed against the
        // merchant's own Campaigns page, which shows real allocations here).
        Campaign.aggregate([
          {
            $group: {
              _id: null,
      ...
      const scratchTotals = scratchAgg[0] || { totalAllocated: 0, totalUsed: 0 };
      const totalCustomers = customerAgg[0]?.mobiles.length || 0;
      ...
      return {
        ...
        platform: {
          ...
          activeCampaigns,
          totalCustomers,
          totalRedemptions: redeemedAgg,
          scratchAllocated: scratchTotals.totalAllocated,
          scratchUsed: scratchTotals.totalUsed,
          scratchRemaining: Math.max(0, scratchTotals.totalAllocated - scratchTotals.totalUsed),
        },
        health: {
          expiringSubscriptions30d: expiringSubsCount,
        },
        timestamp: new Date(),
      };
    } catch (error) {
  ...
  async getRetailerDashboard(merchantId) {
    try {
      ...
      // Fetch all independent data in parallel — cuts sequential DB round-trips
      const [
        subscription,
        stores,
        campaigns,
        ...
        Store.find({ merchant_id: merchantId }).select(
          "store_name address city state latitude longitude scratchCards createdAt contact_person status"
        ),
        Campaign.find({ merchantId }),
        CustomerParticipation.countDocuments({ merchant_id: merchantObjId }),
        CustomerParticipation.countDocuments({ merchant_id: merchantObjId, createdAt: { $gte: thirtyDaysAgo } }),
        CustomerParticipation.countDocuments({ merchant_id: merchantObjId, createdAt: { $gte: startOfMonth } }),
        CustomerParticipation.countDocuments({ merchant_id: merchantObjId, status: { $in: ["verified", "scratched", "revealed", "redeemed"] } }),
        CustomerParticipation.countDocuments({ merchant_id: merchantObjId, status: "redeemed" }),
        CustomerParticipation.countDocuments({ merchant_id: merchantObjId, status: { $in: ["revealed", "redeemed"] } }),
      ]);
      ...
      for (const campaign of campaigns) {
        for (const assign of campaign.assignedStores || []) {
          ...
          // redeemed is a subset of used — only count used_scratch_cards to avoid double-count
          allocationByStore[key].used += assign.used_scratch_cards || 0;
        }
      }
      ...
      const scratchStatus = await scratchEntitlementService.getDashboardStatus(merchantId, "merchant");
  ...
  async getCampaignConsumption(merchantId) {
    const campaigns = await Campaign.find({ merchantId: toObjId(merchantId) })
      .select("campaignName used_scratch_cards allocated_scratch_cards")
      .lean();
    ...
    return (campaigns || [])
      .map((c) => ({
        campaignId: c._id,
        name: c.campaignName,
        // used_scratch_cards already includes all consumed cards (both scratched
        // and redeemed). redeemed_scratch_cards is a subset — adding them would
        // double-count every redeemed card.
  ...
  /**
   * Platform-wide daily scratch card usage time-series (all merchants) — the
   * same real ScratchCardTransaction source as getDailyScratchUsage, just
   * without the merchant_id filter.
   * @returns {Promise<Array<{date:string, used:number}>>} exactly `days` buckets
   */
  async getPlatformDailyScratchUsage({ days = 30 } = {}) {
    const since = startOfWindow(days);
    const rows = await ScratchCardTransaction.aggregate([
      {
        $match: {
  ...
  /**
   * Platform-wide per-campaign scratch consumption (all merchants), sorted
   * desc, filtered to > 0 — same source as getCampaignConsumption without
   * the merchantId filter.
   * @returns {Promise<Array<{campaignId:any, name:string, used:number, allocated:number}>>}
   */
  async getPlatformCampaignConsumption({ limit = 5 } = {}) {
  ...
  /**
   * Platform-wide per-store scratch consumption (all merchants), sorted
   * desc — same source as getStoreWisePerformance without the merchant_id
   * filter.
   * @returns {Promise<Array<{storeId:any, name:string, used:number}>>}
   */
  async getPlatformStoreWisePerformance({ limit = 5 } = {}) {
    const rows = await CampaignStoreMapping.aggregate([
      {
        $group: {
          _id: "$store_id",
