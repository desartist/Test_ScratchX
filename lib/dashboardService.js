...
class DashboardService {
  ...
  async getRetailerDashboard(merchantId) {
    try {
      ...
      // Scratches distributed = any participation that consumed inventory (all non-failed/expired)
      const scratchesUsed = await CustomerParticipation.countDocuments({
        merchant_id: merchantObjId,
        status: { $in: ["verified", "scratched", "revealed", "redeemed"] },
      });
      // Scratches claimed = only those that were actually redeemed by the customer
      const scratchesClaimed = await CustomerParticipation.countDocuments({
        merchant_id: merchantObjId,
        status: "redeemed",
      });
