  ...
  async getRetailerDashboard(merchantId) {
    try {
      ...
      const [
        ...
        // Role check runs alongside the data reads rather than as a
        // sequential round-trip in front of them (checked right after).
        Account.findById(merchantId),
        ...
        // One pass over the merchant's participations instead of six
        // separate countDocuments round-trips.
        CustomerParticipation.aggregate([
          { $match: { merchant_id: merchantObjId } },
          {
            $group: {
              ...
              scratchesUsed: {
                $sum: {
                  $cond: [{ $in: ["$status", ["verified", "scratched", "revealed", "redeemed"]] }, 1, 0],
                },
              },
              scratchesClaimed: {
              ...
              coupons: {
                $sum: { $cond: [{ $in: ["$status", ["revealed", "redeemed"]] }, 1, 0] },
              },
            },
          },
        ]),
